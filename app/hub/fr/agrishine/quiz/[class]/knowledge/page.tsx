...
import { P2_QUIZ } from '@/app/lib/quiz-p2'
import { P3_QUIZ } from '@/app/lib/quiz-p3'
...
import { JSS2_QUIZ } from '@/app/lib/quiz-jss2'
import { JSS3_QUIZ } from '@/app/lib/quiz-jss3'
  ...
  return (
    <main style={{ minHeight:'100vh', fontFamily:'Inter, system-ui, sans-serif', color:'#fff',
      ...
      <section style={{ maxWidth:1080, margin:'0 auto', padding:'44px 26px 8px', textAlign:'center' }}>
        <div style={{ fontFamily:'IBM Plex Mono, monospace', fontSize:13, letterSpacing:'.16em', color:T.goldSoft, marginBottom:12 }}>KNOWLEDGE QUIZ · {klass.replace('primary-','PRIMARY ').replace('jss-','JSS ').toUpperCase()}</div>
        <h1 style={{ fontFamily:'Fraunces, serif', fontWeight:500, fontSize:'clamp(30px,4vw,46px)', margin:'0 0 12px', lineHeight:1.08 }}>Choose a challenge</h1>
        <p style={{ color:'rgba(255,255,255,.82)', fontSize:17, lineHeight:1.6, maxWidth:560, margin:'0 auto' }}>Learn • Play • Score • Earn Badges</p>
      </section>

      <section style={{ maxWidth:1080, margin:'0 auto', padding:'32px 26px 64px' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px, 1fr))', gap:20 }}>
          {(klass === 'jss-3' ? JSS3_QUIZ : klass === 'jss-2' ? JSS2_QUIZ : klass === 'jss-1' ? JSS1_QUIZ : klass === 'primary-6' ? P6_QUIZ : klass === 'primary-5' ? P5_QUIZ : klass === 'primary-4' ? P4_QUIZ : klass === 'primary-3' ? P3_QUIZ : klass === 'primary-2' ? P2_QUIZ : P1_QUIZ).map((cat) => {
            ...
            <div key={cat.slug} onClick={() => router.push(`/hub/fr/agrishine/quiz/${klass}/knowledge/${cat.slug}`)}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:14 }}>
                ...
                <div>
                  <div style={{ fontFamily:'Fraunces, serif', fontSize:23, color:T.ink, lineHeight:1.1 }}>{cat.name}</div>
