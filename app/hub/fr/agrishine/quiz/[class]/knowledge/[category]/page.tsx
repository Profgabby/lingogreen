...
import { p4CategoryBySlug } from '@/app/lib/quiz-p4'
...
const CORRECT_MSGS = ['Great job! 🌟','Excellent! 🎉','Well done! ⭐','You got it! 👏','Amazing! 🌱']
const WRONG_MSGS = ['Good try!','Almost! Let\u2019s learn it.','Keep going!']

// shuffle options but keep track of which is correct
function shuffled(q: QuizQuestion) {
  const idx = q.options.map((_, i) => i)
  for (let i = idx.length - 1; i > 0; i--) { const j = Math.floor(Math.random()*(i+1)); [idx[i],idx[j]]=[idx[j],idx[i]] }
  return { options: idx.map(i => q.options[i]), correct: idx.indexOf(q.correct) }
}

const DIFF = { easy:{label:'EASY',dot:'🟢'}, medium:{label:'MEDIUM',dot:'🟡'}, hard:{label:'HARD',dot:'🔴'} }
...
export default function QuizPlayer() {
  ...
  const cat = klass === 'primary-6' ? p6CategoryBySlug(catSlug) : klass === 'primary-5' ? p5CategoryBySlug(catSlug) : klass === 'primary-4' ? p4CategoryBySlug(catSlug) : klass === 'primary-3' ? p3CategoryBySlug(catSlug) : klass === 'primary-2' ? p2CategoryBySlug(catSlug) : categoryBySlug(catSlug)
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((s) => {
        if (s <= 1) { clearInterval(timerRef.current!); handleTimeout(); return 0 }
        return s - 1
      })
    }, 1000)
  ...
  if (!cat) {
    return (<main style={{ minHeight:'100vh', display:'grid', placeItems:'center', background:T.forest, color:'#fff', fontFamily:'Inter' }}><div>Quiz not found.</div></main>)
  }
  if (checking) {
  ...
  function advance(nextMisses: number) {
    setTimeout(() => {
      if (nextMisses >= 3) { setEnded('terminated'); return }
      if (qi + 1 >= cat!.questions.length) { setEnded('complete'); return }
      setQi(qi + 1); setPicked(null); setLocked(false); setFeedback('')
    }, 1400)
  }
  ...
  function pick(i: number) {
    if (locked) return
    timerRef.current && clearInterval(timerRef.current)
    setLocked(true); setPicked(i)
    if (i === d.correct) {
      setScore(s => s + 10); setCorrect(c => c + 1); setMisses(0)
      setFeedback(CORRECT_MSGS[Math.floor(Math.random()*CORRECT_MSGS.length)])
      advance(0)
    } else {
      const nm = misses + 1
      setWrong(w => w + 1); setMisses(nm)
      setFeedback(WRONG_MSGS[Math.floor(Math.random()*WRONG_MSGS.length)])
      advance(nm)
    }
  }
  ...
  if (ended) {
    ...
    const accuracy = completed ? Math.round((correct / completed) * 100) : 0
    const earnedBadge = ended === 'complete' && misses < 3
    let stars = '🌱 Keep Learning'
    if (score >= 450) stars = '⭐⭐⭐ Outstanding'
    else if (score >= 350) stars = '⭐⭐ Great Work'
    else if (score >= 250) stars = '⭐ Good Effort'

    return (
      <main style={{ minHeight:'100vh', fontFamily:'Inter, system-ui, sans-serif', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', padding:'40px 20px',
        background:`linear-gradient(165deg, ${T.forest}, ${T.forest2})` }}>
        <div style={{ background:'#fff', color:T.ink, borderRadius:24, maxWidth:440, width:'100%', padding:'34px 30px', textAlign:'center', boxShadow:'0 24px 60px -20px rgba(0,0,0,.5)', borderTop:`8px solid ${cat.color}` }}>
          ...
          <h1 style={{ fontFamily:'Fraunces, serif', fontWeight:600, fontSize:28, margin:'0 0 6px' }}>
            {ended === 'complete' ? 'Quiz Complete!' : 'Good Try!'}
          </h1>
          {ended === 'terminated' && <p style={{ color:T.ink2, fontSize:15, margin:'0 0 16px' }}>You missed three questions in a row. Keep learning and come back again!</p>}
          <div style={{ fontFamily:'IBM Plex Mono, monospace', fontSize:12, color:T.muted, letterSpacing:'.08em', marginBottom:4 }}>{cat.name.toUpperCase()}</div>

          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, margin:'18px 0' }}>
            <Stat label="Score" value={`${score} / 500`} />
            <Stat label="Correct" value={`${correct} / 50`} />
            <Stat label="Completed" value={`${completed} / 50`} />
            <Stat label="Accuracy" value={`${accuracy}%`} />
          </div>

          <div style={{ fontFamily:'Fraunces, serif', fontSize:18, color:cat.color, marginBottom:14 }}>{stars}</div>

          {earnedBadge && (
            <div style={{ background:'rgba(62,155,124,.12)', borderRadius:16, padding:'16px', marginBottom:16 }}>
              <div style={{ fontSize:34 }}>{cat.badgeIcon}</div>
              <div style={{ fontFamily:'Fraunces, serif', fontSize:19, color:T.ink, marginTop:4 }}>You earned the</div>
              <div style={{ fontFamily:'Fraunces, serif', fontSize:22, color:cat.color, fontWeight:600 }}>{cat.badge} Badge!</div>
            </div>
          )}
  ...
  // ---------- QUESTION SCREEN ----------
  const pct = Math.round((qi / cat.questions.length) * 100)
  const urgent = timeLeft <= 10
  const diffMeta = DIFF[q.diff]

  return (
    <main style={{ minHeight:'100vh', fontFamily:'Inter, system-ui, sans-serif', color:'#fff',
      background:`linear-gradient(165deg, ${T.forest}, ${T.forest2})`, paddingBottom:40 }}>
      {/* top bar */}
      <div style={{ padding:'16px 22px 0', maxWidth:640, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', fontFamily:'IBM Plex Mono, monospace', fontSize:12.5, color:'rgba(255,255,255,.8)' }}>
          <span>Question {qi + 1} of {cat.questions.length}</span>
          <span>{diffMeta.dot} {diffMeta.label}</span>
        </div>
      </div>
      ...
      <div style={{ maxWidth:640, margin:'14px auto 0', padding:'0 22px', display:'flex', gap:10, justifyContent:'space-between' }}>
        ...
        <MiniStat label="Misses" value={'●'.repeat(misses) + '○'.repeat(3 - misses)} />
        <MiniStat label="Time" value={`00:${String(timeLeft).padStart(2,'0')}`} urgent={urgent} />
      </div>
      ...
      <div style={{ maxWidth:640, margin:'20px auto 0', padding:'0 22px' }}>
        <div style={{ background:'#fff', color:T.ink, borderRadius:20, padding:'26px 24px', boxShadow:'0 16px 40px -18px rgba(0,0,0,.5)', borderTop:`6px solid ${cat.color}` }}>
          <div style={{ fontFamily:'Fraunces, serif', fontSize:24, lineHeight:1.25, marginBottom:22, minHeight:60 }}>{q.q}</div>
          <div style={{ display:'grid', gap:12 }}>
            {d.options.map((opt, i) => {
              let bg = '#F4EFE4', color = T.ink, border = '2px solid transparent'
              if (locked) {
                if (i === d.correct) { bg = 'rgba(62,155,124,.18)'; border = `2px solid ${T.green}` }
                else if (i === picked) { bg = 'rgba(214,96,74,.15)'; border = `2px solid ${T.red}` }
              }
              return (
                <button key={i} onClick={() => pick(i)} disabled={locked}
                  style={{ textAlign:'left', background:bg, border, borderRadius:14, padding:'16px 18px', fontSize:18, fontWeight:500, cursor:locked?'default':'pointer', fontFamily:'Inter', color, display:'flex', alignItems:'center', gap:12 }}>
          ...
          {feedback && (
            <div style={{ marginTop:18, textAlign:'center', fontFamily:'Fraunces, serif', fontSize:20,
              color: picked !== null && picked === d.correct ? T.green : (feedback==='Time\u2019s up!'? T.red : T.ink2) }}>
              {feedback}
              {locked && picked !== d.correct && <div style={{ fontSize:14.5, color:T.ink2, marginTop:4, fontFamily:'Inter' }}>Correct answer: <b>{d.options[d.correct]}</b></div>}
            </div>
          )}
        </div>
      </div>
...
function MiniStat({ label, value, urgent }: { label:string; value:React.ReactNode; urgent?:boolean }) {
  return (
    <div style={{ flex:1, background: urgent?'rgba(214,96,74,.25)':'rgba(255,255,255,.08)', borderRadius:12, padding:'8px 6px', textAlign:'center', border: urgent?'1px solid rgba(214,96,74,.6)':'1px solid rgba(255,255,255,.12)' }}>
      ...
      <div style={{ fontFamily:'IBM Plex Mono, monospace', fontSize:16, fontWeight:600, color: urgent?'#ffd9d2':'#fff', marginTop:2 }}>{value}</div>
    </div>
  )
}
...
function Stat({ label, value }: { label:string; value:string }) {
  return (
    <div style={{ background:'#F4EFE4', borderRadius:12, padding:'12px 8px' }}>
      ...
      <div style={{ fontFamily:'Fraunces, serif', fontSize:20, color:'#2A2118', marginTop:2 }}>{value}</div>
    </div>
  )
}
