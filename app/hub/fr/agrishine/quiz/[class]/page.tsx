  ...
  return (
    <main style={{ minHeight:'100vh', fontFamily:'Inter, system-ui, sans-serif', color:'#fff',
      ...
      <section style={{ maxWidth:1080, margin:'0 auto', padding:'44px 26px 8px', textAlign:'center' }}>
        <div style={{ fontFamily:'IBM Plex Mono, monospace', fontSize:13, letterSpacing:'.16em', color:T.goldSoft, marginBottom:12 }}>PRIMARY 1 LEARNING QUIZ</div>
      ...
      <section style={{ maxWidth:1080, margin:'0 auto', padding:'32px 26px 64px' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px, 1fr))', gap:20 }}>
          {P1_QUIZ.map((cat) => (
            <div key={cat.slug} onClick={() => router.push(`/hub/fr/agrishine/quiz/${klass}/${cat.slug}`)}
              ...
              <button onClick={(e)=>{ e.stopPropagation(); router.push(`/hub/fr/agrishine/quiz/${klass}/${cat.slug}`) }}
